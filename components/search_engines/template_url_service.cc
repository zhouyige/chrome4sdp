...
TemplateURLService::TemplateURLService(
  ...
  if (profile_ && profile_->IsOffTheRecord()) {
    synced_default_search_provider_guid_ = prefs::kSyncedDefaultSearchProviderGUIDIncognito;
  } else {
    synced_default_search_provider_guid_ = prefs::kSyncedDefaultSearchProviderGUID;
  }
...
void TemplateURLService::RegisterProfilePrefs(
  ...
  registry->RegisterStringPref(prefs::kSyncedDefaultSearchProviderGUID,
                               std::string(),
                               flags);
  registry->RegisterStringPref(prefs::kSyncedDefaultSearchProviderGUIDIncognito,
                               std::string(),
                               flags);
...
void TemplateURLService::RepairPrepopulatedSearchEngines(bool search_engines_injected) {
...
void TemplateURLService::Init(const Initializer* initializers,
  ...
  if (prefs_) {
    ...
    pref_change_registrar_.Add(
      synced_default_search_provider_guid_,
      base::Bind(
...
void TemplateURLService::MaybeUpdateDSEAfterSync(TemplateURL* synced_turl) {
  if (prefs_ &&
      (synced_turl->sync_guid() ==
          prefs_->GetString(synced_default_search_provider_guid_))) {
    default_search_manager_.SetUserSelectedDefaultSearchEngine(
        synced_turl->data());
  }
}
...
void TemplateURLService::OnDefaultSearchChange(
  ...
  if (prefs_ && (source == DefaultSearchManager::FROM_USER) &&
    ...
    prefs_->SetString(synced_default_search_provider_guid_, data->sync_guid);
  }
...
bool TemplateURLService::ApplyDefaultSearchChangeNoMetrics(
  ...
  if (!data) {
    ...
    if (default_search_provider_ && prefs_) {
      prefs_->SetString(synced_default_search_provider_guid_,
