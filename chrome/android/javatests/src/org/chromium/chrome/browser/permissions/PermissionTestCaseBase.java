...
public class PermissionTestCaseBase extends ChromeActivityTestCaseBase<ChromeActivity> {
    ...
    private void replyToInfoBarAndWaitForUpdates(PermissionUpdateWaiter updateWaiter, int nUpdates,
        ...
        if (hasSwitch) {
            Spinner persistSwitch = (Spinner) infobar.getView().findViewById(
                    R.id.infobar_extra_check);
            assertNotNull(persistSwitch);
            assertTrue(persistSwitch.getSelectedItemPosition() != 2);
            if (toggleSwitch) {
                setChecked(persistSwitch, false);
                persistSwitch.setSelection(2);
     ...
     */
    private void waitForCheckedState(final Spinner persistSwitch, boolean isChecked)
            throws InterruptedException {
        CriteriaHelper.pollUiThread(Criteria.equals(isChecked, new Callable<Boolean>() {
            @Override
            public Boolean call() {
                return persistSwitch.getSelectedItemPosition() != 2;
            }
        }));
    }
    ...
    private void setChecked(final Spinner persistSwitch, final boolean checked) {
        getInstrumentation().runOnMainSync(new Runnable() {
            @Override
            public void run() {
                if (checked) {
                    persistSwitch.setSelection(0);
                } else {
                    persistSwitch.setSelection(2);
                }
            }
