...
public class BookmarkPage extends BasicNativePage 
    ...
    private Runnable mModelLoadRunnable = new Runnable() {
        @Override
        public void run() {
            Uri defaultBookmarks = BookmarkUIState
                    .createFolderUrl(mManager.getModel().getDefaultFolder());
            updateForUrl(defaultBookmarks.toString());
        }
    };
    ...
    public void updateForUrl(String url) {
        // avoid looping forever by converting the basic bookmark url into a folder specific one.
        if (UrlConstants.BOOKMARKS_URL.equals(url)) {
            mManager.getModel().runAfterBookmarkModelLoaded(mModelLoadRunnable);
