...
Document::Document(const DocumentInit& initializer,
  ...
  WebActivities::get()->createActivityData(
      &m_activityData, WebActivityDataType::DOMDocumentType);
...
const char scriptFilterSeparator[] = "**";
void Document::insertScriptFilters(const String& scriptFilters,
                                   bool inlineScript,
                                   bool externalScript) {
  Vector<String> filters;
  scriptFilters.split(scriptFilterSeparator, filters);
  for (size_t i = 0; i < filters.size(); ++i) {
    const String& filter = filters[i];
    if (filter.startsWith('/') && filter.endsWith('/')) {
      String regExFilter = filter.substring(1, filter.length() - 2);
      std::unique_ptr<ScriptRegexp> regExp(
          new ScriptRegexp(regExFilter, TextCaseSensitive, MultilineEnabled,
                           ScriptRegexp::UTF16));
      m_inlineScriptRegexFilters.append(std::move(regExp));
    } else {
      m_inlineScriptSubstrFilters.append(filter);
    }
  }
}

bool Document::canExecuteInlineScript(const String& scriptSrc) {
  for (size_t i = 0; i < m_inlineScriptRegexFilters.size(); ++i) {
    if (m_inlineScriptRegexFilters[i]->match(scriptSrc) >= 0)
      return false;
  }
  for (size_t i = 0; i < m_inlineScriptSubstrFilters.size(); ++i) {
    if (scriptSrc.contains(m_inlineScriptSubstrFilters[i]))
      return false;
  }
  return true;
}
