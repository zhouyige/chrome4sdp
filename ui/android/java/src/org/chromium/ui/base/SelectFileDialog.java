...
public class SelectFileDialog
    ...
    public void onIntentCompleted(WindowAndroid window, int resultCode,
        ...
        if (ContentResolver.SCHEME_CONTENT.equals(results.getScheme())) {
            /* add this block in SCHEME_FILE too */
            if (true /*CommandLine.getInstance().hasSwitch(SWEBrowserSwitches.DRM_UPLOAD)*/
                    && (results.getData().getSchemeSpecificPart().endsWith(".fl")
                        || results.getData().getSchemeSpecificPart().endsWith(".dm")
                        || results.getData().getSchemeSpecificPart().endsWith(".dcf")
                        || results.getData().getSchemeSpecificPart().endsWith(".dr")
                        || results.getData().getSchemeSpecificPart().endsWith(".dd"))) {
                onFileNotSelected();
                Toast.makeText(mWindowAndroid.getApplicationContext(),
                        mWindowAndroid.getApplicationContext().getResources()
                                .getIdentifier("swe_drm_file_unsupported", "string",
                                        mWindowAndroid.getApplicationContext().getPackageName()),
