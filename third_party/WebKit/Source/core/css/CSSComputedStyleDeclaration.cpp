...
const CSSValue* CSSComputedStyleDeclaration::getPropertyCSSValue(
  ...
  if (ancestor && !isTreeScopeRoot(ancestor) && !styledNode->isInShadowTree() &&
      !styledNode->parentElementShadow() &&
      !styledNode->containingShadowRoot() && styledNode->isElementNode() &&
      styledNode->parentComputedStyle()) {
    bool canDoStrictRecalc = false;
    if (ancestor->needsStyleRecalc() &&
        (ancestor->styleChangeState() ==
         ChildrenAffectedByBackForwardPositionalRules))
      canDoStrictRecalc = true;
    else if (!ancestor->needsStyleRecalc()) {
      bool invalidateAncestorStyle = false;
      for (Node* a = ancestor; a && !invalidateAncestorStyle;
           a = a->parentOrShadowHostNode())
        invalidateAncestorStyle =
            a->needsStyleRecalc() || a->childNeedsDistributionRecalc();
    ...
    if (canDoStrictRecalc) {
      ...
      if (styledNode->needsStyleRecalc() &&
        ...
        DocumentLifecycle::LifecycleState oldState =
        ...
        document.ensureStyleResolver();
        {
          ...
          styledElement->recalcStyle(NoChange, nullptr, true);
        }
