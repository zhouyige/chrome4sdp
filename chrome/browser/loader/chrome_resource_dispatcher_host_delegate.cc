...
void ChromeResourceDispatcherHostDelegate::AppendStandardResourceThrottles(
  ...
  content::ResourceThrottle* second_throttle = NULL;
  //Check if XSS Defender is enabled
  if (!second_throttle && xss_defender::XSSDefender::IsEnabled() &&
    ...
    if (site_setting != CONTENT_SETTING_BLOCK) {
      second_throttle = XSSResourceThrottle::MaybeCreate(
          request, resource_type, io_data->IsOffTheRecord());
    }
  }

  if (second_throttle)
    throttles->push_back(second_throttle);
