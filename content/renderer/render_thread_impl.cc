...
scoped_refptr<ContextProviderCommandBuffer> CreateOffscreenCanvasContext(
    scoped_refptr<gpu::GpuChannelHost> gpu_channel_host,
    const gpu::SharedMemoryLimits& limits,
    bool support_locking,
    command_buffer_metrics::ContextType type,
    int32_t stream_id,
    gpu::GpuStreamPriority stream_priority,
    ContextProviderCommandBuffer* share_context) {
  DCHECK(gpu_channel_host);
  // This is used to create a few different offscreen contexts:
  // - The shared main thread context (offscreen) used by blink for canvas.
  // - The worker context (offscreen) used for GPU raster and video decoding.
  ...
  return make_scoped_refptr(new ContextProviderCommandBuffer(
      ...
      gpu::kNullSurfaceHandle,
      GURL("chrome://gpu/RenderThreadImpl::CreateOffscreenContext"),
      automatic_flushes, support_locking, limits,
      attributes, share_context, type));
}
...
RenderThreadImpl::OffscreenCanvasContextProvider() {
  ...
  if (offscreen_canvas_contexts_ &&
      offscreen_canvas_contexts_->ContextGL()->GetGraphicsResetStatusKHR() ==
          GL_NO_ERROR)
    return offscreen_canvas_contexts_;

  scoped_refptr<gpu::GpuChannelHost> gpu_channel_host(EstablishGpuChannelSync());
  if (!gpu_channel_host) {
    offscreen_canvas_contexts_ = nullptr;
    return nullptr;
  }

  bool support_locking = true;
  int32_t stream_id = gpu::GPU_STREAM_DEFAULT;//gpu_channel_host->GenerateStreamID();

  offscreen_canvas_contexts_ = CreateOffscreenCanvasContext(
      std::move(gpu_channel_host), gpu::SharedMemoryLimits(), support_locking,
      command_buffer_metrics::OFFSCREEN_CANVASTHREAD_CONTEXT,
      stream_id, gpu::GpuStreamPriority::NORMAL,
      shared_main_thread_contexts_.get());

  if (!offscreen_canvas_contexts_->BindToCurrentThread()) {
    offscreen_canvas_contexts_ = nullptr;
  }
  return offscreen_canvas_contexts_;
}
