...
void DriverGL::InitializeStaticBindings() {
  ...
  fn.glCompressTextureFn = reinterpret_cast<glCompressTextureProc>(
      GetGLProcAddress("glCompressTexture"));
...
static void GL_BINDING_CALL Debug_glCompressTexture(void) {
  GL_SERVICE_LOG("glCompressTexture"
                 << "("
                 << ")");
  DCHECK(g_driver_gl.debug_fn.glCompressTextureFn != nullptr);
  g_driver_gl.debug_fn.glCompressTextureFn();
}
...
static void GL_BINDING_CALL Debug_glEndTilingQCOM(GLbitfield preserveMask) {
  GL_SERVICE_LOG("glEndTilingQCOM"
                 << "(" << preserveMask << ")");
  DCHECK(g_driver_gl.debug_fn.glEndTilingQCOMFn != nullptr);
...
static void GL_BINDING_CALL Debug_glStartTilingQCOM(GLuint x,
  ...
  GL_SERVICE_LOG("glStartTilingQCOM"
                 << "(" << x << ", " << y << ", " << width << ", " << height
                 << ", " << preserveMask << ")");
  DCHECK(g_driver_gl.debug_fn.glStartTilingQCOMFn != nullptr);
...
void DriverGL::InitializeDebugBindings() {
  ...
  if (!debug_fn.glCompressTextureFn) {
    debug_fn.glCompressTextureFn = fn.glCompressTextureFn;
    fn.glCompressTextureFn = Debug_glCompressTexture;
  }
...
void GLApiBase::glCompressTextureFn(void) {
  driver_->fn.glCompressTextureFn();
}
...
void TraceGLApi::glCompressTextureFn(void) {
  TRACE_EVENT_BINARY_EFFICIENT0("gpu", "TraceGLAPI::glCompressTexture")
  gl_api_->glCompressTextureFn();
}
...
void NoContextGLApi::glCompressTextureFn(void) {
  NOTREACHED()
      << "Trying to call glCompressTexture() without current GL context";
  LOG(ERROR) << "Trying to call glCompressTexture() without current GL context";
}
