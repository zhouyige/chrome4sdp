...
public class ChildProcessLauncher {
    ...
    private static final String NUM_SANDBOXED_SERVICES_IF_NO_PROP_KEY =
            "org.chromium.content.browser.NUM_SANDBOXED_SERVICES_IF_NO_PROP";
    ...
    private static String getSysProp(String propName) {
        ...
        try {
            ...
            value = (String)m.invoke(null, propName);
        } catch (Exception e) {
            Log.w(TAG, "Failed to find bg_apps_limit, assuming no_prop", e);
        }

        return value;
    }

    private static boolean hasSysBgAppsLimitProp() {
        String sysPropBgAppLimit = getSysProp("ro.sys.fw.bg_apps_limit");
        if (sysPropBgAppLimit == null || sysPropBgAppLimit.isEmpty()) {
            return false;
        } else {
            Log.i(TAG, "ro.sys.fw.bg_app_limit = " + sysPropBgAppLimit);
            return true;
        }
    }
    ...
    private static int getNumberOfServices(Context context, boolean inSandbox, String packageName) {
        ...
        if (inSandbox
            ...
            try {
                ...
                if (appInfo.metaData != null) {
                    String numSandboxedServicesKeyName = NUM_SANDBOXED_SERVICES_KEY;
                    if (appInfo.metaData.containsKey(NUM_SANDBOXED_SERVICES_IF_NO_PROP_KEY) &&
                            !hasSysBgAppsLimitProp()) {
                        numSandboxedServicesKeyName = NUM_SANDBOXED_SERVICES_IF_NO_PROP_KEY;
                    }

                    numServices = appInfo.metaData.getInt(inSandbox
                            ? numSandboxedServicesKeyName : NUM_PRIVILEGED_SERVICES_KEY, -1);
                }
        ...
        Log.i(TAG, "numServices = " + numServices);
        return numServices;
    }
