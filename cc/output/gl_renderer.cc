...
GLRenderer::GLRenderer(const RendererSettings* settings,
  ...
  using_swap_buffer_with_damage_ = context_caps.use_tiled_rendering && context_caps.post_sub_buffer;
#else
  using_swap_buffer_with_damage_ = context_caps.post_sub_buffer;
#endif
#endif

#ifdef DO_TILED_RENDERING
  use_tiled_rendering_ = context_caps.use_tiled_rendering;
#endif
...
void GLRenderer::DrawTileQuad(const DrawingFrame* frame,
  ...
  if (!settings_->disable_ubwc &&
      compress_count_ < 2 &&
      quad->texture_size.width() <= settings_->ubwc_max_size &&
      quad->texture_size.height() <= settings_->ubwc_max_size &&
      resource_provider_->NeedsCompression(quad->resource_id(),
        quad->draw_count, settings_->ubwc_threshold)) {
    gl_->CompressTexture();
    compress_count_++;
    resource_provider_->DidCompress(quad->resource_id());
  }
}
...
#ifdef DO_TILED_RENDERING
  if (frame->root_render_pass == render_pass && use_tiled_rendering_) {
