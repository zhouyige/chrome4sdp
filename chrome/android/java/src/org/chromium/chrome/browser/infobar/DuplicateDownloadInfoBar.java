...
public class DuplicateDownloadInfoBar extends ConfirmInfoBar {
    ...
    private static InfoBar createInfoBar(
            String filePath, long totalBytes, String mimeType, boolean isOfflinePage,
            String pageUrl, boolean isIncognito) {
        if (DownloadInfoBar.areSWEDownloadEnhancementsEnabled()) {
            return new DownloadInfoBar(ContextUtils.getApplicationContext(), filePath,
                    isOfflinePage, pageUrl, isIncognito, totalBytes, mimeType);
        } else {
     ...
     */
    protected DuplicateDownloadInfoBar(
    ...
    protected native void nativeSetDirFullPath(long nativeDuplicateDownloadInfoBar,
            String dirFullPath);
}
