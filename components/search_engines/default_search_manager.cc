...
const char DefaultSearchManager::kDefaultSearchProviderDataPrefNameIncognito[] =
    "default_search_provider_data.template_url_data_incognito";
...
DefaultSearchManager::DefaultSearchManager(
  ...
  if (pref_service_) {
    ...
    if (profile_ && profile_->IsOffTheRecord()){
        default_search_provider_data_pref_name_ = kDefaultSearchProviderDataPrefNameIncognito;
    } else {
        default_search_provider_data_pref_name_ = kDefaultSearchProviderDataPrefName;
    }

    pref_change_registrar_.Add(
        default_search_provider_data_pref_name_,
...
void DefaultSearchManager::RegisterProfilePrefs(
  ...
  registry->RegisterDictionaryPref(kDefaultSearchProviderDataPrefNameIncognito);
}
...
void DefaultSearchManager::AddPrefValueToMap(
  ...
  pref_value_map->SetValue(kDefaultSearchProviderDataPrefName,
                           std::move(value));
  pref_value_map->SetValue(kDefaultSearchProviderDataPrefNameIncognito,
                           std::move(value));
}
...
void DefaultSearchManager::SetUserSelectedDefaultSearchEngine(
  ...
  pref_service_->Set(default_search_provider_data_pref_name_, url_dict);
}
...
void DefaultSearchManager::ClearUserSelectedDefaultSearchEngine() {
  if (pref_service_) {
    pref_service_->ClearPref(default_search_provider_data_pref_name_);
  } else {
...
void DefaultSearchManager::LoadDefaultSearchEngineFromPrefs() {
  ...
  pref = pref_service_->FindPreference(default_search_provider_data_pref_name_);

  DCHECK(pref);
  default_search_controlled_by_policy_ = pref->IsManaged();

  const base::DictionaryValue* url_dict;
  url_dict = pref_service_->GetDictionary(default_search_provider_data_pref_name_);
