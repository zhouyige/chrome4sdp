...
void HandleCrashDump(const BreakpadInfo& info) {
...
#endif
  {
    ...
    writer.AddBoundary();
#endif
#ifdef SWE_PACKAGE_NAME
    writer.AddPairString("swe_package_name", SWE_PACKAGE_NAME);
    writer.AddBoundary();
#endif  // #ifdef SWE_PACKAGE_NAME
#ifdef SWE_BRANDING_CHANNEL_NAME
    writer.AddPairString("swe_branding_channel_name", SWE_BRANDING_CHANNEL_NAME);
    writer.AddBoundary();
#endif  // #ifdef SWE_BRANDING_CHANNEL_NAME
  ...
  if (process_type.empty()) {
    ...
    if (!enable_breakpad) {
      enable_breakpad = parsed_command_line.HasSwitch(
        ...
        parsed_command_line.HasSwitch(switches::kCrashLogServerUrl);
    }
    if (!enable_breakpad) {
