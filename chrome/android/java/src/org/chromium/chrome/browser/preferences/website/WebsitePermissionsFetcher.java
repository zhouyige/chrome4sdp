...
public class WebsitePermissionsFetcher {
    ...
    public void fetchAllPreferences() {
        ...
        // XSS Defender permission is per-origin
        queue.add(new XSSDefenderInfoFetcher());
    ...
    public void fetchPreferencesForCategory(SiteSettingsCategory category) {
        ...
        if (category.showGeolocationSites()) {
            ...
            // Secure Connect permission is per-origin
            queue.add(new SecureConnectInfoFetcher());
        } else if (category.showXSSDefenderSites()) {
            // XSS Defender permission is per-origin
            queue.add(new XSSDefenderInfoFetcher());
    ...
    private class XSSDefenderInfoFetcher extends Task {
        @Override
        public void run() {
            for (XSSDefenderInfo info : WebsitePreferenceBridge.getXSSDefenderInfo()) {
                WebsiteAddress origin = WebsiteAddress.create(info.getOrigin());
                if (origin == null) continue;
                WebsiteAddress embedder = WebsiteAddress.create(info.getEmbedder());
                findOrCreateSite(origin, embedder).setXSSDefenderInfo(info);
            }
        }
    }
}
