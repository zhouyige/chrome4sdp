...
public class X509Util {
    ...
    /**
     * Trust manager for enterprise root CA.
     */
    private static X509TrustManagerImplementation sEnterpriseTrustManager;
    private static KeyStore sEnterpriseKeyStore;
    ...
    private static void ensureInitializedLocked()
        ...
        if (sEnterpriseKeyStore == null) {
            sEnterpriseKeyStore = KeyStore.getInstance(KeyStore.getDefaultType());
            try {
                sEnterpriseKeyStore.load(null);
            } catch (IOException e) {
                // No IO operation is attempted.
            }
        }
        if (sEnterpriseTrustManager == null) {
            sEnterpriseTrustManager = X509Util.createTrustManager(sEnterpriseKeyStore);
        }
    ...
    /**
     * After each modification of enterprise key store, trust manager has to be generated again.
     */
    private static void reloadEnterpriseTrustManager() throws KeyStoreException,
            NoSuchAlgorithmException {
        assert Thread.holdsLock(sLock);
    ...
    public static void addEnterpriseRootCertificate(byte[] rootCertBytes) throws CertificateException,
            KeyStoreException, NoSuchAlgorithmException {
        ensureInitialized();
        X509Certificate rootCert = createCertificateFromBytes(rootCertBytes);
        synchronized (sLock) {
            sEnterpriseKeyStore.setCertificateEntry(
                    "root_cert_" + Integer.toString(sEnterpriseKeyStore.size()), rootCert);
            reloadEnterpriseTrustManager();
        }
    }

    public static void clearEnterpriseRootCertificates() throws NoSuchAlgorithmException,
            CertificateException, KeyStoreException {
        ensureInitialized();
        synchronized (sLock) {
            try {
        ...
        synchronized (sLock) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        verifiedChain = sEnterpriseTrustManager.checkServerTrusted(serverCertificates,
                                                                         authType, host);
                    }  catch (CertificateException eEnterpriseManager) {
