...
class TestResourcePrefetcher : public ResourcePrefetcher {
 public:
  TestResourcePrefetcher(ResourcePrefetcher::Delegate* delegate,
                         ...
                         const std::vector<ResourcePrefetcher::URLInfo>& urls)
      : ResourcePrefetcher(delegate, config, navigation_id, main_frame_url, urls) {}
...
class ResourcePrefetcherTest : public testing::Test {
 ...
 protected:
  ...
  void CheckURLRequestID(const std::string& url, NavigationID navigation_id,
                                  content::ResourceType resource_type) {
    net::URLRequest* request = GetInFlightRequest(url);
    EXPECT_NE(request, nullptr);
    content::URLRequestID* url_request_id = content::URLRequestID::ForRequest(request);
    EXPECT_NE(url_request_id, nullptr);

    EXPECT_EQ(url_request_id->GetRenderProcessID(), navigation_id.render_process_id);
    EXPECT_EQ(url_request_id->GetRouteID(), navigation_id.route_id);
    EXPECT_EQ(url_request_id->GetRenderFrameID(), navigation_id.render_frame_id);
    EXPECT_EQ(url_request_id->GetParentRenderFrameID(), navigation_id.parent_render_frame_id);
    EXPECT_EQ(url_request_id->GetResourceType(), resource_type);
  }
...
TEST_F(ResourcePrefetcherTest, TestPrefetcherFinishes) {
  ...
  std::vector<ResourcePrefetcher::URLInfo> urls = {
              {GURL("http://www.google.com/resource1.html"), content::RESOURCE_TYPE_MAIN_FRAME},
              {GURL("http://www.google.com/resource2.png"), content::RESOURCE_TYPE_IMAGE},
              {GURL("http://yahoo.com/resource1.png"), content::RESOURCE_TYPE_IMAGE},
              {GURL("http://yahoo.com/resource2.png"), content::RESOURCE_TYPE_IMAGE},
              {GURL("http://yahoo.com/resource3.png"), content::RESOURCE_TYPE_IMAGE},
              {GURL("http://m.google.com/resource1.jpg"), content::RESOURCE_TYPE_IMAGE},
              {GURL("http://www.google.com/resource3.html"), content::RESOURCE_TYPE_MAIN_FRAME},
              {GURL("http://m.google.com/resource2.html"), content::RESOURCE_TYPE_MAIN_FRAME},
              {GURL("http://m.google.com/resource3.css"), content::RESOURCE_TYPE_STYLESHEET},
              {GURL("http://m.google.com/resource4.png"), content::RESOURCE_TYPE_IMAGE},
              {GURL("http://yahoo.com/resource4.png"), content::RESOURCE_TYPE_IMAGE},
              {GURL("http://yahoo.com/resource5.png"), content::RESOURCE_TYPE_IMAGE}};

  NavigationID navigation_id(1, MSG_ROUTING_NONE, 2, -1, main_frame_url);

  prefetcher_.reset(new TestResourcePrefetcher(&prefetcher_delegate_, config_,
                                               navigation_id, main_frame_url, urls));
  ...
  CheckURLRequestID("http://www.google.com/resource1.html", navigation_id,
                                          content::RESOURCE_TYPE_MAIN_FRAME);
  CheckURLRequestID("http://www.google.com/resource2.png", navigation_id,
                                          content::RESOURCE_TYPE_IMAGE);
  CheckURLRequestID("http://yahoo.com/resource1.png", navigation_id,
                                          content::RESOURCE_TYPE_IMAGE);
