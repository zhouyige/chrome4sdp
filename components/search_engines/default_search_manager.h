...
class DefaultSearchManager {
 public:
  ...
  static const char kDefaultSearchProviderDataPrefNameIncognito[];
  ...
  DefaultSearchManager(PrefService* pref_service,
                       const ObserverCallback& change_observer, Profile* profile = NULL);
 ...
 private:
  ...
  // Holds the regular or incognito pref name as per the profile
  const char *default_search_provider_data_pref_name_;
