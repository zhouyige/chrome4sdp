...
void GLES2DecoderTestBase::InitDecoderWithCommandLine(
  ...
  EXPECT_CALL(*gl_, GenTextures(1, _))
      ...
      .RetiresOnSaturation();
  EXPECT_CALL(*gl_, BindTexture(GL_TEXTURE_2D, 5))
      .Times(1)
      .RetiresOnSaturation();
  EXPECT_CALL(*gl_, TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, _, _, 0,
        GL_RGBA, GL_UNSIGNED_BYTE, nullptr))
      .Times(1)
      .RetiresOnSaturation();

  EXPECT_CALL(*gl_, GenFramebuffersEXT(1, _))
      .WillOnce(SetArgPointee<1>(6))
      .RetiresOnSaturation();
  EXPECT_CALL(*gl_, BindFramebufferEXT(GL_FRAMEBUFFER, 6))
      .Times(1)
      .RetiresOnSaturation();
  EXPECT_CALL(*gl_, FramebufferTexture2DEXT(GL_FRAMEBUFFER,
        GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 5, 0))
      .Times(1)
      .RetiresOnSaturation();

  EXPECT_CALL(*gl_, BindFramebufferEXT(GL_DRAW_FRAMEBUFFER, 3))
    .Times(1)
    .RetiresOnSaturation();
  EXPECT_CALL(*gl_, BindFramebufferEXT(GL_READ_FRAMEBUFFER, 4))
    .Times(1)
    .RetiresOnSaturation();
  EXPECT_CALL(*gl_, BindTexture(GL_TEXTURE_2D, 2))
    .Times(1)
    .RetiresOnSaturation();
