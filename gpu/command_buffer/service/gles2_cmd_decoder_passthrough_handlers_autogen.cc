...
error::Error GLES2DecoderPassthroughImpl::HandleStartTilingQCOM(
    uint32_t immediate_data_size,
    const volatile void* cmd_data) {
  const volatile gles2::cmds::StartTilingQCOM& c =
      *static_cast<const volatile gles2::cmds::StartTilingQCOM*>(cmd_data);
  GLuint x = static_cast<GLuint>(c.x);
  GLuint y = static_cast<GLuint>(c.y);
  GLuint width = static_cast<GLuint>(c.width);
  GLuint height = static_cast<GLuint>(c.height);
  GLbitfield preserveMask = static_cast<GLbitfield>(c.preserveMask);
  error::Error error = DoStartTilingQCOM(x, y, width, height, preserveMask);
  if (error != error::kNoError) {
    return error;
  }
  return error::kNoError;
...
error::Error GLES2DecoderPassthroughImpl::HandleEndTilingQCOM(
    uint32_t immediate_data_size,
    const volatile void* cmd_data) {
  const volatile gles2::cmds::EndTilingQCOM& c =
      *static_cast<const volatile gles2::cmds::EndTilingQCOM*>(cmd_data);
  GLbitfield preserveMask = static_cast<GLbitfield>(c.preserveMask);
  error::Error error = DoEndTilingQCOM(preserveMask);
  if (error != error::kNoError) {
    return error;
  }
  return error::kNoError;
...
error::Error GLES2DecoderPassthroughImpl::HandleCompressTexture(
    uint32_t immediate_data_size,
    const volatile void* cmd_data) {
  error::Error error = DoCompressTexture();
  if (error != error::kNoError) {
    return error;
  }
  return error::kNoError;
