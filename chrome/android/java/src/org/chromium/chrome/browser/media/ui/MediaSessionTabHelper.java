...
public class MediaSessionTabHelper implements MediaImageCallback {
    ...
    private MediaNotificationListener mControlsListener = new MediaNotificationListener() {
        ...
        @Override
        public void onDeltaSeek(int actionSource, int delta) {
            MediaSessionUMA
                    .recordDeltaSeek(MediaSessionTabHelper.convertMediaActionSourceToUMA(actionSource));

            if (mMediaSessionObserver.getMediaSession() != null) {
                mMediaSessionObserver.getMediaSession().deltaSeek(delta);
            }
        }
