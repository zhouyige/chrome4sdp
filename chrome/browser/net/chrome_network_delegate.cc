...
int ChromeNetworkDelegate::OnBeforeURLRequest(
  ...
  if (info && (info->GetResourceType() == content::RESOURCE_TYPE_SUB_FRAME)) {
    ProfileIOData* io_data =
        ProfileIOData::FromResourceContext(info->GetContext());
    ContentSetting site_setting = io_data->GetHostContentSettingsMap()->
        GetContentSetting(request->url(), request->url(),
            CONTENT_SETTINGS_TYPE_XSS_DEFENDER, std::string());

    if (xss_defender::XSSDefender::IsEnabled() && site_setting != CONTENT_SETTING_BLOCK) {
      if (request->url().has_query() && xss_defender::XSSDefender::IsMalicious(
          request->url(), info->GetResourceType(), io_data->IsOffTheRecord())) {
        // Filter Iframe URL from XSS scripts
        GURL filteredURL(xss_defender::XSSDefender::FilterURL(request->url()));
        new_url->Swap(&filteredURL);
      }
    }
  }
