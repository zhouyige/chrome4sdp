...
void CopyUrls(const std::vector<ResourcePrefetcher::URLInfo>& urls,
  std::vector<GURL>& dst_urls) {
  dst_urls.clear();
  for (const auto& info : urls)
    dst_urls.push_back(info.url_);
}
...
TEST_F(ResourcePrefetchPredictorTest, PopulatePrefetcherRequest) {
  ...
  EXPECT_TRUE(predictor_->PopulatePrefetcherRequest(google.primary_key(),
                                                    test_data, &urls));
  CopyUrls(urls, gurls);
  EXPECT_THAT(gurls, UnorderedElementsAre(GURL("http://google.com/image1.png"),
...
TEST_F(ResourcePrefetchPredictorTest, GetPrefetchData) {
  ...
  CopyUrls(urls, gurls);
  EXPECT_THAT(gurls, UnorderedElementsAre(GURL(script_url)));
  ...
  EXPECT_TRUE(predictor_->GetPrefetchData(main_frame_url, &urls));
  CopyUrls(urls, gurls);
  EXPECT_THAT(gurls, UnorderedElementsAre(GURL(image_url)));
  ...
  EXPECT_TRUE(predictor_->GetPrefetchData(main_frame_url, &urls));
  CopyUrls(urls, gurls);
  EXPECT_THAT(gurls, UnorderedElementsAre(GURL(style_url)));
  ...
  EXPECT_TRUE(predictor_->GetPrefetchData(main_frame_url, &urls));
  CopyUrls(urls, gurls);
  EXPECT_THAT(gurls, UnorderedElementsAre(GURL(font_url)));
}
