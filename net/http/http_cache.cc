...
void HttpCache::OnBackendCreated(int result, PendingOp* pending_op) {
  ...
  if (!item->DoCallback(result, disk_cache_.get())) {
    ...
    if (libswenet_manager) {
        if (!libswenet_manager->GetIoMessageLoop()) {
          libswenet_manager->SetIoMessageLoop( base::MessageLoopForIO::current());
        }
        if (!libswenet_manager->GetHttpCache()) {
          libswenet_manager->SetHttpCache(this);
        }
    }
  }
}
