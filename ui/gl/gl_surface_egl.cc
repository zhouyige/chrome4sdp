...
#ifdef DO_TILED_RENDERING
  EGLint preserved = 0;
  if (gl::g_driver_gl.ext.b_GL_QCOM_tiled_rendering) {
...
bool NativeViewGLSurfaceEGL::Initialize(
  ...
  if (gl::g_driver_egl.ext.b_EGL_KHR_swap_buffers_with_damage) {
  ...
  if (!g_gl_tiled_rendering_queried) {
    ...
    if (!context) {
      ...
      scoped_refptr<GLContext> new_context = InitializeGLContext(
        new GLContextEGL(nullptr), this, GLContextAttribs());
      if (!new_context.get()) {
