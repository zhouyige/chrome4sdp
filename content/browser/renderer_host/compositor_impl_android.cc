...
void CompositorImpl::CreateLayerTreeHost() {
  ...
  settings.renderer_settings.disable_ubwc =
    command_line->HasSwitch(cc::switches::kDisableUbwc);
  if (command_line->HasSwitch(cc::switches::kUbwcThreshold)) {
    std::string threshold_str =
      command_line->GetSwitchValueASCII(cc::switches::kUbwcThreshold);
    int threshold;
    if (base::StringToInt(threshold_str, &threshold))
      settings.renderer_settings.ubwc_threshold = threshold;
  }
  if (command_line->HasSwitch(cc::switches::kUbwcMaxSize)) {
    std::string size_str =
      command_line->GetSwitchValueASCII(cc::switches::kUbwcMaxSize);
    int size;
    if (base::StringToInt(size_str, &size) && size >= 64 && size <= 2048)
      settings.renderer_settings.ubwc_max_size = size;
  }
