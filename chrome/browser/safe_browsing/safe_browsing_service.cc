...
SafeBrowsingService::SafeBrowsingService()
    : services_delegate_(ServicesDelegate::Create(this)),
      ...
      cellular_enabled_by_prefs_(false) {}
...
void SafeBrowsingService::StartOnIOThread(
  ...
  if (enabled_) {
    #if defined(SAFE_BROWSING_DB_LOCAL)
      protocol_manager_->SetCellularUpdates(cellular_updates);
    #endif
    return;
  }
  enabled_ = true;
...
void SafeBrowsingService::AddPrefService(PrefService* pref_service) {
  ...
  registrar->Add(prefs::kSecurityUpdatesCellularAllowed,
                 base::Bind(&SafeBrowsingService::RefreshState,
                            base::Unretained(this)));
...
void SafeBrowsingService::RefreshState() {
  ...
  bool enable = false;
  bool cellular_updates = false;
  for (const auto& pref : prefs_map_) {
    if (pref.first->GetBoolean(prefs::kSafeBrowsingEnabled) && !enable) {
      enable = true;
    }
    if (pref.first->GetBoolean(prefs::kSecurityUpdatesCellularAllowed)
        && !cellular_updates) {
      cellular_updates = true;
    }
  }

  enabled_by_prefs_ = enable;
  cellular_enabled_by_prefs_ = cellular_updates;
