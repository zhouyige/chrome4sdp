...
ResourcePrefetcher::URLInfo::URLInfo(const GURL& url, content::ResourceType type)
  : url_(url),
    type_(type) {
}

ResourcePrefetcher::URLInfo::URLInfo(const URLInfo& other)
  : url_(other.url_),
    type_(other.type_) {
}
  ...
  if (state_ == RUNNING) {
    ...
    while ((inflight_requests_.size() <
      ...
      for (; request_it != request_queue_.end(); ++request_it) {
        const std::string& host = request_it->url_.host();
...
void ResourcePrefetcher::SendRequest(const URLInfo& url_info) {
  const GURL& url = url_info.url_;
  ...
  content::URLRequestID* url_request_id = new content::URLRequestID(
                                        ...
                                        url_info.type_);
  url_request_id->AssociateWithRequest(url_request.get());
