...
bool MediaStreamInfoBarDelegateAndroid::Handle(int action) {
  if (action != InfoBarAndroid::ACTION_OK_24H) {
    return action == InfoBarAndroid::ACTION_OK ? Accept() : Cancel();
  }
  ...
  controller_->set_persist(persist_permission);
  controller_->PermissionGranted_24H();
  return true;
}
