...
void GL_BINDING_CALL MockGLInterface::Mock_glCompressTexture(void) {
  MakeFunctionUnique("glCompressTexture");
  interface_->CompressTexture();
}
...
void* GL_BINDING_CALL MockGLInterface::GetGLProcAddress(const char* name) {
  ...
  if (strcmp(name, "glCompressTexture") == 0)
    return reinterpret_cast<void*>(Mock_glCompressTexture);
