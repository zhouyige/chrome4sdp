...
TEST(URLMatcherTest, TestConditionSetsHaveSingleCondition) {
  GURL url1("http://www.example.com:1234/foo");
  GURL url2("http://www.google.com/foo");
  URLMatcher matcher1(true), matcher2;
  URLMatcherConditionFactory factory;
  URLMatcherConditionSet::Vector vector;
  URLMatcherCondition m1 = factory.CreatePathContainsCondition("foo");
  URLMatcherCondition m2 = factory.CreateHostEqualsCondition("www.example.com");
  URLMatcherCondition m3 = factory.CreateHostPrefixCondition("www");
  URLMatcherCondition m4 = factory.CreateHostSuffixCondition("google.com");

  std::set<URLMatcherCondition> conditions;
  conditions.insert(m1);
  scoped_refptr<URLMatcherConditionSet> condition_set1(
  ...
  ranges.push_back(URLMatcherPortFilter::CreateRange(8080));
  scoped_refptr<URLMatcherConditionSet> condition_set2(
      new URLMatcherConditionSet(2,
                                 conditions,
                                 std::unique_ptr<URLMatcherSchemeFilter>(),
                                 std::unique_ptr<URLMatcherPortFilter>(new URLMatcherPortFilter(ranges))));

  conditions.clear();
  conditions.insert(m2);
  scoped_refptr<URLMatcherConditionSet> condition_set3(
      new URLMatcherConditionSet(3, conditions));

  conditions.clear();
  conditions.insert(m3);
  conditions.insert(m4);
  scoped_refptr<URLMatcherConditionSet> condition_set4(
      new URLMatcherConditionSet(4, conditions));

  vector.push_back(condition_set1);
  vector.push_back(condition_set2);
  vector.push_back(condition_set3);
  vector.push_back(condition_set4);

  matcher1.AddConditionSets(vector);
  matcher2.AddConditionSets(vector);

  std::set<URLMatcherConditionSet::ID> idset1 = {1u, 2u, 3u, 4u};
  std::set<URLMatcherConditionSet::ID> idset2 = {3u};
  std::set<URLMatcherConditionSet::ID> idset3 = {1u, 2u, 4u};
  std::set<URLMatcherConditionSet::ID> idset4 = {4u};
  EXPECT_EQ(idset1, matcher1.MatchURL(url1));
  EXPECT_EQ(idset2, matcher2.MatchURL(url1));
  EXPECT_EQ(idset3, matcher1.MatchURL(url2));
  EXPECT_EQ(idset4, matcher2.MatchURL(url2));
}

class URLMatcherTest {
 public:
    static void TestClearURLMatcherConditionSetsAfterUse();
    static void TestClearURLMatcherConditionFactoryAfterUse();
};

void URLMatcherTest::TestClearURLMatcherConditionSetsAfterUse() {
  ...
  conditions.insert(m2);
  scoped_refptr<URLMatcherConditionSet> condition_set2(
      new URLMatcherConditionSet(2, conditions));
  ...
  EXPECT_EQ(0u, matcher1.url_matcher_condition_sets_.size());
  EXPECT_EQ(2u, matcher2.url_matcher_condition_sets_.size());

  std::set<URLMatcherConditionSet::ID> idset = {1u, 2u};
  EXPECT_EQ(idset, matcher1.MatchURL(url));
  EXPECT_EQ(idset, matcher2.MatchURL(url));
}

void URLMatcherTest::TestClearURLMatcherConditionFactoryAfterUse() {
  GURL url("http://www.example.com/foo");
  URLMatcher matcher(true);
  URLMatcherConditionFactory* factory = matcher.condition_factory();
  URLMatcherConditionSet::Vector vector;
  URLMatcherCondition m1 = factory->CreatePathContainsCondition("foo");
  URLMatcherCondition m2 = factory->CreateHostEqualsCondition("www.example.com");
  URLMatcherCondition m3 = factory->CreateURLMatchesCondition("http://www.ex*");
  ...
  EXPECT_EQ(2u, factory->substring_pattern_singletons_.size());
  EXPECT_EQ(1u, factory->regex_pattern_singletons_.size());
  matcher.AddConditionSets(vector);
  EXPECT_EQ(0u, factory->substring_pattern_singletons_.size());
  EXPECT_EQ(0u, factory->regex_pattern_singletons_.size());

  std::set<URLMatcherConditionSet::ID> idset = {1u, 2u, 3u};
  EXPECT_EQ(idset, matcher.MatchURL(url));
}

TEST(URLMatcherTest, TestClearURLMatcherConditionSetsAfterUse) {
    URLMatcherTest::TestClearURLMatcherConditionSetsAfterUse();
}

TEST(URLMatcherTest, TestClearURLMatcherConditionFactoryAfterUse) {
    URLMatcherTest::TestClearURLMatcherConditionFactoryAfterUse();
}
