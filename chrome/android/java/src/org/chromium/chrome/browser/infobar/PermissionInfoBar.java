...
public class PermissionInfoBar
    ...
    private static PermissionInfoBar create(Tab tab, int enumeratedIconId, Bitmap iconBitmap,
            ...
            int[] contentSettingsTypes, boolean showPersistenceToggle, boolean isIncognito) {
        int drawableId = ResourceId.mapToDrawableId(enumeratedIconId);

        boolean create24HInfoBar = false;
        if (!isIncognito) {
            create24HInfoBar = true;
            for (int setting : contentSettingsTypes) {
                if (setting != ContentSettingsType.CONTENT_SETTINGS_TYPE_GEOLOCATION &&
                        setting != ContentSettingsType.CONTENT_SETTINGS_TYPE_MEDIASTREAM_CAMERA &&
                        setting != ContentSettingsType.CONTENT_SETTINGS_TYPE_MEDIASTREAM_MIC) {
                    create24HInfoBar = false;
                }
            }
        }

        PermissionInfoBar infoBar = null;
        if (create24HInfoBar) {
            infoBar = new BrowserPermissionInfoBar(tab, contentSettingsTypes, drawableId,
                iconBitmap, message, linkText, buttonOk, buttonCancel, showPersistenceToggle);
        } else {
            infoBar = new PermissionInfoBar(tab, contentSettingsTypes, drawableId,
                iconBitmap, message, linkText, buttonOk, buttonCancel, showPersistenceToggle);
        }
