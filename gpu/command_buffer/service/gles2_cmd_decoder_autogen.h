...
error::Error GLES2DecoderImpl::HandleStartTilingQCOM(
  ...
  DoStartTilingQCOM(x, y, width, height, preserveMask);
  return error::kNoError;
}

error::Error GLES2DecoderImpl::HandleEndTilingQCOM(
    uint32_t immediate_data_size,
    const volatile void* cmd_data) {
  const volatile gles2::cmds::EndTilingQCOM& c =
      *static_cast<const volatile gles2::cmds::EndTilingQCOM*>(cmd_data);
  GLbitfield preserveMask = static_cast<GLbitfield>(c.preserveMask);
  DoEndTilingQCOM(preserveMask);
  return error::kNoError;
}

error::Error GLES2DecoderImpl::HandleCompressTexture(
    uint32_t immediate_data_size,
    const volatile void* cmd_data) {
  DoCompressTexture();
  return error::kNoError;
