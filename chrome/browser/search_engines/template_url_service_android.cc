...
Profile* GetOffTheRecordProfile() {
  return GetOriginalProfile()->GetOffTheRecordProfile();
}
...
void TemplateUrlServiceAndroid::ClearPrepopulatedEnginesInPrefs() {
  ...
  template_url_service_->RepairPrepopulatedSearchEngines(true);
  overrides.Clear();
}

void TemplateUrlServiceAndroid::AddSearchEngine(JNIEnv* env,
    jobject obj, jstring jtitle, jstring jkeyword,
    jstring jsearch_url, jstring jfavicon_url, jstring jcontextual_search_url) {
  ...
  entry->SetString("contextual_search_url",base::android::ConvertJavaStringToUTF16(env,
    jcontextual_search_url));
...
static jlong Init(JNIEnv* env, const JavaParamRef<jobject>& obj, jboolean is_incognito) {
  TemplateUrlServiceAndroid* template_url_service_android =
      new TemplateUrlServiceAndroid(env, obj, is_incognito);
