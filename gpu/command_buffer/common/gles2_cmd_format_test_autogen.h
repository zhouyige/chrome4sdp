...
TEST_F(GLES2FormatTest, CompressTexture) {
  cmds::CompressTexture& cmd = *GetBufferAs<cmds::CompressTexture>();
  void* next_cmd = cmd.Set(&cmd);
  EXPECT_EQ(static_cast<uint32_t>(cmds::CompressTexture::kCmdId),
            cmd.header.command);
  EXPECT_EQ(sizeof(cmd), cmd.header.size * 4u);
  CheckBytesWrittenMatchesExpectedSize(next_cmd, sizeof(cmd));
}
