...
void Element::recalcStyle(StyleRecalcChange change, Text* nextTextSibling, bool scoped) {
  ...
  if (change >= IndependentInherit || needsStyleRecalc()) {
    if (hasRareData()) {
      ...
      if (!scoped && change >= IndependentInherit) {
        if (ElementAnimations* elementAnimations = data->elementAnimations())
          elementAnimations->setAnimationStyleChange(false);
      }
    }
    if (parentComputedStyle())
      change = recalcOwnStyle(change, nextTextSibling, scoped);
    if (scoped)
...
StyleRecalcChange Element::recalcOwnStyle(StyleRecalcChange change,
                                  Text* nextTextSibling, bool scoped) {
  DCHECK(document().inStyleRecalc());
  DCHECK(scoped || !parentOrShadowHostNode()->needsStyleRecalc());
  ...
  if (!newStyle) {
    newStyle = !scoped ? styleForLayoutObject()
                       : document().ensureStyleResolver().styleForElement(
                             this, 0, DisallowStyleSharing);
  }
  DCHECK(newStyle);
