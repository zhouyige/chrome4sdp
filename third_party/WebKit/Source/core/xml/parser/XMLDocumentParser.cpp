...
void XMLDocumentParser::append(const String& inputSource) {
  ...
  if (document()->isWMLDocument() && !m_parsedFirstWMLFragment) {
    ...
    DEFINE_STATIC_LOCAL(String, xslInsertion,
                        ("<?xml-stylesheet type=\"text/xsl\" "
                         "href=\"chrome://resources/wml/swe_wml.xsl\"?>\n"));
...
static bool shouldAllowExternalLoad(const KURL& url) {
  ...
  if (XMLDocumentParserScope::currentDocument->isWMLDocument()) {
    if (urlString.startsWith("http://www.wapforum.org/DTD",
                             TextCaseInsensitive) ||
        urlString.startsWith("http://www.openwave.com/DTD",
                             TextCaseInsensitive) ||
