...
void GetOrigins(JNIEnv* env,
  ...
  for (const auto& settings_it : all_settings) {
    if (settings_it.setting == default_content_setting &&
        content_type != CONTENT_SETTINGS_TYPE_SECURE_CONNECT)
      continue;
    if (managedOnly &&
...
static void GetSecureConnectOrigins(JNIEnv* env,
                                 const JavaParamRef<jclass>& clazz,
                                 const JavaParamRef<jobject>& list,
                                 jboolean managedOnly) {
  GetOrigins(env, CONTENT_SETTINGS_TYPE_SECURE_CONNECT,
    &Java_WebsitePreferenceBridge_insertSecureConnectInfoIntoList,
    list, managedOnly);
}

static jint GetSecureConnectSettingForOrigin(JNIEnv* env,
                                          const JavaParamRef<jclass>& clazz,
                                          const JavaParamRef<jstring>& origin,
                                          const JavaParamRef<jstring>& embedder,
                                          jboolean is_incognito) {
  return GetSettingForOrigin(env, CONTENT_SETTINGS_TYPE_SECURE_CONNECT, origin,
                             embedder, is_incognito);
}

static void SetSecureConnectSettingForOrigin(JNIEnv* env,
                                          const JavaParamRef<jclass>& clazz,
                                          const JavaParamRef<jstring>& origin,
                                          const JavaParamRef<jstring>& embedder,
                                          jint value,
                                          jboolean is_incognito) {
  SetSettingForOrigin(env, CONTENT_SETTINGS_TYPE_SECURE_CONNECT, origin, nullptr,
                      static_cast<ContentSetting>(value), is_incognito);
}

static void GetXSSDefenderOrigins(JNIEnv* env,
                                 const JavaParamRef<jclass>& clazz,
                                 const JavaParamRef<jobject>& list,
                                 jboolean managedOnly) {
  GetOrigins(env, CONTENT_SETTINGS_TYPE_XSS_DEFENDER,
    &Java_WebsitePreferenceBridge_insertXSSDefenderInfoIntoList,
    list, managedOnly);
}

static jint GetXSSDefenderSettingForOrigin(JNIEnv* env,
                                          const JavaParamRef<jclass>& clazz,
                                          const JavaParamRef<jstring>& origin,
                                          const JavaParamRef<jstring>& embedder,
                                          jboolean is_incognito) {
  return GetSettingForOrigin(env, CONTENT_SETTINGS_TYPE_XSS_DEFENDER, origin,
                             embedder, is_incognito);
}

static void SetXSSDefenderSettingForOrigin(JNIEnv* env,
                                          const JavaParamRef<jclass>& clazz,
                                          const JavaParamRef<jstring>& origin,
                                          const JavaParamRef<jstring>& embedder,
                                          jint value,
                                          jboolean is_incognito) {
  SetSettingForOrigin(env, CONTENT_SETTINGS_TYPE_XSS_DEFENDER, origin, nullptr,
                      static_cast<ContentSetting>(value), is_incognito);
}
