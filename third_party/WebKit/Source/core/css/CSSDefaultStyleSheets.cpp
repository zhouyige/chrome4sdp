...
void CSSDefaultStyleSheets::addDefaultExtraStyleSheet01(
    const String& styleRules) {
  if (!m_defaultExtraStyle01) {
    m_defaultExtraStyle01 = RuleSet::create();
    m_defaultExtraStyleSheet01 = parseUASheet(styleRules);
    m_defaultExtraStyle01->addRulesFromSheet(defaultExtraStyleSheet01(),
                                             screenEval());
  }
}

void CSSDefaultStyleSheets::addDefaultExtraStyleSheet02(
    const String& styleRules) {
  if (!m_defaultExtraStyle02) {
    m_defaultExtraStyle02 = RuleSet::create();
    m_defaultExtraStyleSheet02 = parseUASheet(styleRules);
    m_defaultExtraStyle02->addRulesFromSheet(defaultExtraStyleSheet02(),
                                             screenEval());
  }
}
