...
bool ChromePasswordManagerClient::DecryptMatch(const autofill::PasswordForm* preferred_match) {
  bool result = false;
  ...
  if (!base64_encrypted_str.empty()) {
    ...
    if (result) {
      const_cast<autofill::PasswordForm *>(preferred_match)->password_value = decrypted_str;
    }
