...
public class MinidumpUploadCallable implements Callable<Integer> {
    ...
    @VisibleForTesting
    protected static final String CRASH_LOG_SERVER_CMD = "crash-log-server";
    ...
    public MinidumpUploadCallable(
            File fileToUpload, File logfile, CrashReportingPermissionManager permissionManager, Context context) {
        this(fileToUpload, logfile, new HttpURLConnectionFactoryImpl(), permissionManager, context);
    ...
    public MinidumpUploadCallable(File fileToUpload, File logfile,
            ...
            CrashReportingPermissionManager permissionManager, Context context) {
    ...
    public Integer call() {
        ...
        if (CommandLine.getInstance().hasSwitch(CRASH_LOG_SERVER_CMD)) {
            CRASH_URL_STRING = CommandLine.getInstance().getSwitchValue(CRASH_LOG_SERVER_CMD);
        } else {
    ...
    private boolean configureConnectionForHttpPost(HttpURLConnection connection)
        ...
        Log.e(TAG, "configureConnectionForHttpPost enter");
        String boundary = readBoundary();
        if (boundary == null) {
            Log.e(TAG, "configureConnectionForHttpPost readBoundary failed!");
            return false;
        }
        ...
        Log.e(TAG, "configureConnectionForHttpPost Content-Type : " + String.format(CONTENT_TYPE_TMPL, boundary));
        connection.setRequestProperty("Content-Type", String.format(CONTENT_TYPE_TMPL, boundary));
        if (!TextUtils.isEmpty(mApiKey))
            connection.setRequestProperty("x-api-key", mApiKey);

        Log.e(TAG, "configureConnectionForHttpPost exit");
        return true;
    }
