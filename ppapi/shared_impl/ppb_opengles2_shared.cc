...
void StartTilingQCOM(PP_Resource context_id,
                     GLuint x,
                     GLuint y,
                     GLuint width,
                     GLuint height,
  ...
  Enter3D enter(context_id, true);
  if (enter.succeeded()) {
    ToGles2Impl(&enter)->StartTilingQCOM(x, y, width, height, preserveMask);
  }
}

void EndTilingQCOM(PP_Resource context_id, GLbitfield preserveMask) {
  Enter3D enter(context_id, true);
  if (enter.succeeded()) {
    ToGles2Impl(&enter)->EndTilingQCOM(preserveMask);
  }
}

void CompressTexture(PP_Resource context_id) {
  Enter3D enter(context_id, true);
  if (enter.succeeded()) {
    ToGles2Impl(&enter)->CompressTexture();
  }
}
