...
public final class PrefServiceBridge {
    ...
    public static final String SECURE_CONTENT_ONLY_PREFERENCE = "secure_content_only_preference";
    public static final String SECURE_CONTENT_ONLY_FOR_NETWORK_PREFERENCE =
            "secure_content_only_for_network_preference";
    public static final String WEB_DEFENDER_PREFERENCE = "web_defender_preference";

    private Set<String> mOriginsPendingReload = new HashSet<>();
    private boolean mForceGlobalReload = false;
    private HashMap<String, Boolean> mTemporarySecureConnectMainFramePreferences = new HashMap<>();
    private HashMap<String, Boolean> mTemporarySecureConnectRulesetPreferences = new HashMap<>();
    private HashMap<String, Integer> mTemporaryTrackerDomainPreferences = new HashMap<>();
    ...
    public void setTemporarySecureConnectMainFramePreferences(HashMap<String, Boolean> toCache) {
        mTemporarySecureConnectMainFramePreferences = toCache;
    }
    public HashMap<String, Boolean> getTemporarySecureConnectMainFramePreferences() {
        return mTemporarySecureConnectMainFramePreferences;
    }

    public void setTemporarySecureConnectRulesetPreferences(HashMap<String, Boolean> toCache) {
        mTemporarySecureConnectRulesetPreferences = toCache;
    }
    public HashMap<String, Boolean> getTemporarySecureConnectRulesetPreferences() {
        return mTemporarySecureConnectRulesetPreferences;
    }

    public void setTemporaryTrackerDomainPreferences(HashMap<String, Integer> toCache) {
        mTemporaryTrackerDomainPreferences = toCache;
    }
    public HashMap<String, Integer> getTemporaryTrackerDomainPreferences() {
        return mTemporaryTrackerDomainPreferences;
    }

    public void purgeTemporaryPrefs() {
        mTemporaryTrackerDomainPreferences.clear();
        mTemporarySecureConnectMainFramePreferences.clear();
        mTemporarySecureConnectRulesetPreferences.clear();
    }

    public void addOriginForReload(String origin) {
        mOriginsPendingReload.add(origin);
    }

    public Set<String> getOriginsPendingReload() {
        return mOriginsPendingReload;
    }

    public boolean getPendingReload() {
        return mForceGlobalReload;
    }

    public void requestReload() {
        mForceGlobalReload = true;
    }

    public void reloadComplete() {
        mForceGlobalReload = false;
        mOriginsPendingReload.clear();
    }
    ...
    public void setWebDefenderEnabled(boolean enabled) {
        if (!userDefinedWebDefender()) {
            SharedPreferences sharedPreferences =
                    ContextUtils.getAppSharedPreferences();
            // Write that the user has modified the preference
    ...
    /**
     * Sets Secure Content preference.
     */
    public void setSecureContentOnlyEnabled(boolean enabled) {
        SharedPreferences sharedPreferences = ContextUtils.getAppSharedPreferences();
        sharedPreferences.edit()
                .putBoolean(PrefServiceBridge.SECURE_CONTENT_ONLY_PREFERENCE, enabled)
                .apply();
        // Secure content enabled for network takes precedence over global setting
        // if secure content for network feature is enabled
        if (isSecureContentOnlyForNetworkDisabled() || enabled) {
            SecureConnectPreferenceHandler.setSecureContentOnlyMode(enabled);
        }
    }

    /**
     * @return whether Secure Content for Network feature is disabled, either with command line
     * switch or through UI. If feature is disabled, user won't be prompted to enable secure content
     * mode while connecting to new network and the saved preference for a network won't be used
     * if one exists.
     */
    public boolean isSecureContentOnlyForNetworkDisabled() {
        return CommandLine.getInstance()
                .hasSwitch(ChromeSwitches.DISABLE_SECURE_CONTENT_NETWORK) ||
                !getSecureContentOnlyForNetworkEnabled();
    }

    /**
     * @return whether Secure Content preference is enabled.
     */
    public boolean getSecureContentOnlyEnabled() {
        SharedPreferences sharedPreferences = ContextUtils.getAppSharedPreferences();
        return sharedPreferences
                .getBoolean(PrefServiceBridge.SECURE_CONTENT_ONLY_PREFERENCE, false);
    }

    /**
     * Sets Secure Content for Network preference
     */
    public void setSecureContentOnlyForNetworkEnabled(boolean enabled) {
        SharedPreferences sharedPreferences = ContextUtils.getAppSharedPreferences();
        sharedPreferences.edit()
                .putBoolean(PrefServiceBridge.SECURE_CONTENT_ONLY_FOR_NETWORK_PREFERENCE, enabled)
                .apply();
        if(enabled) {
            // Use network setting if available
            Boolean pref = SecureConnectNetworkObserver.getSecureContentForNetwork();
            if(pref != null) {
                SecureConnectPreferenceHandler
                        .setSecureContentOnlyMode(getSecureContentOnlyEnabled());
            }
        } else {
            // Go back to global preference
            SecureConnectPreferenceHandler.setSecureContentOnlyMode(getSecureContentOnlyEnabled());
        }
    }

    /**
     * @return whether Secure Content for Network preference is enabled.
     */
    public boolean getSecureContentOnlyForNetworkEnabled() {
        SharedPreferences sharedPreferences = ContextUtils.getAppSharedPreferences();
        return sharedPreferences
                .getBoolean(PrefServiceBridge.SECURE_CONTENT_ONLY_FOR_NETWORK_PREFERENCE, true);
    }
