...
bool ResourceProvider::NeedsCompression(ResourceId id,
                                        int draw_count,
                                        int threshold) {
  Resource* resource = GetResource(id);
  DCHECK(resource);
  ...
  // An old Resource may have been associated with a new tile.
  // Usually this is indicated with a low draw count.  Make sure
  // the compressed flag is cleared in this case.
  resource->compressed = false;
  return false;
}

void ResourceProvider::DidCompress(ResourceId id) {
  Resource* resource = GetResource(id);
  DCHECK(resource);
