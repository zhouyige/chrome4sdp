...
class GLES2DecoderImpl : public GLES2Decoder, public ErrorStateClient {
 ...
 private:
  ...
  // Compress the currently bound texture
  void DoCompressTexture(void);
  ...
  void DoStartTilingQCOM(GLuint x, GLuint y, GLuint width, GLuint height, GLbitfield preserveMask);
  void DoEndTilingQCOM(GLbitfield preserveMask);
...
#define GLES2_CMD_OP(name) \
  ...
  GLuint ubwc_texture_;
  GLuint ubwc_framebuffer_;
...
bool GLES2DecoderImpl::Initialize(
  ...
  // Create FBO for UBWC conversion
  // record state
  GLint old_texture;
  ...
  // Create texture to back FBO
  glGenTextures(1, &ubwc_texture_);
  DCHECK(ubwc_texture_ != 0);
  glBindTexture(GL_TEXTURE_2D, ubwc_texture_);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, MAX_UBWC_SIZE, MAX_UBWC_SIZE, 0,
      GL_RGBA, GL_UNSIGNED_BYTE, nullptr);

  // Create FBO and attach backing texture
  glGenFramebuffersEXT(1, &ubwc_framebuffer_);
  DCHECK(ubwc_framebuffer_ != 0);
  glBindFramebufferEXT(GL_FRAMEBUFFER, ubwc_framebuffer_);
  glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
      GL_TEXTURE_2D, ubwc_texture_, 0);
...
void GLES2DecoderImpl::Destroy(bool have_context) {
  ...
  //delete UBWC related texture and framebuffer.
  glDeleteFramebuffersEXT(1,&ubwc_framebuffer_);
  glDeleteTextures(1,&ubwc_texture_);
...
void GLES2DecoderImpl::DoCompressTexture() {
  TRACE_EVENT0("gpu", "GLES2DecoderImpl::DoCompressTexture");
  // Make sure a texture is currently bound to GL_TEXTURE_2D
  TextureUnit& unit = state_.texture_units[state_.active_texture_unit];
  scoped_refptr<TextureRef> texture_ref = unit.GetInfoForTarget(GL_TEXTURE_2D);
  if (texture_ref) {
    Texture* texture = texture_ref->texture();
    if (texture) {
      GLsizei width = 0, height = 0;
      texture->GetLevelSize(GL_TEXTURE_2D, 0, &width, &height, nullptr);
      if (width <= MAX_UBWC_SIZE && height <= MAX_UBWC_SIZE) {
        GLint draw_framebuffer, read_framebuffer;
        glGetIntegerv(GL_DRAW_FRAMEBUFFER_BINDING, &draw_framebuffer);
        glGetIntegerv(GL_READ_FRAMEBUFFER_BINDING, &read_framebuffer);
        glBindFramebufferEXT(GL_FRAMEBUFFER, ubwc_framebuffer_);

        // bind current texture to framebuffer
        glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
            GL_TEXTURE_2D, texture->service_id(), 0);
        // copy from current texture into framebuffer texture
        glBindTexture(GL_TEXTURE_2D, ubwc_texture_);
        glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 0, 0, width, height, 0);

        // bind framebuffer texture to framebuffer
        glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
            GL_TEXTURE_2D, ubwc_texture_, 0);
        glBindTexture(GL_TEXTURE_2D, texture->service_id());

#if 0
        __android_log_print(ANDROID_LOG_WARN, __FUNCTION__, "Hello %d %d x %d",
            texture->service_id(), width, height);
        // For testing purposes, mark compressed tiles with yellow square
        // save state
        ...
        // state_.SetDeviceCapabilityState(GL_SCISSOR_TEST, scissor_enabled);
#endif

        // copy from framebuffer texture into current texture
        glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 0, 0, width, height, 0);
...
void GLES2DecoderImpl::DoStartTilingQCOM(GLuint x, GLuint y, GLuint width, GLuint height, GLbitfield preserveMask) {
#ifdef DO_TILED_RENDERING
  ...
  if (!surface_->CanDoTiling()) {
    ZEROCOPY_LOG_PARTIAL_COMPOSITION("GLES2DecoderImpl::DoStartTilingQCOM not tiling");
    return;
  }
  ...
  ZEROCOPY_LOG_PARTIAL_COMPOSITION("GLES2DecoderImpl::DoStartTilingQCOM tiling %d, %d, %d %d", x, y, width, height);
  glStartTilingQCOM(x, y, width, height, preserveMask);
#endif
}

void GLES2DecoderImpl::DoEndTilingQCOM(GLbitfield preserveMask) {
#ifdef DO_TILED_RENDERING
  if (!features().use_tiled_rendering) {
    LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "glStartTilingQCOM",
                       "function not available");
