 ...
 */
public class WebRefinerTestServer {
    private static final String TAG = "WebRefinerTestServer";
     ...
     * @throws Exception
     */
    private WebRefinerTestServer(int port, boolean ssl) throws Exception {
        mPort = port;
    ...
    public static WebRefinerTestServer start(int port) throws Exception {
        WebRefinerTestServer server = new WebRefinerTestServer(port, false);
        server.mServerThread.start();
        return server;
    }

    public static WebRefinerTestServer start() throws Exception {
        return start(0);
    }

    public static WebRefinerTestServer startSsl(int port) throws Exception {
        WebRefinerTestServer server = new WebRefinerTestServer(port, true);
        server.mServerThread.start();
        return server;
    }

    public static WebRefinerTestServer startSsl() throws Exception {
        return startSsl(0);
    }
     ...
     * the client calling {@link WebRefinerTestServer#shutdown()} can issue a request
     * for shutdown by blindly trusting the {@link WebRefinerTestServer}'s
     * credentials.
     */
    private static class TestTrustManager implements X509TrustManager {
        ...
        public void checkClientTrusted(X509Certificate[] chain, String authType) {
            // Trust the WebRefinerTestServer...
        }
     ...
     * {@link WebRefinerTestServer#shutdown()}.
     */
    ...
    private HttpResponse createResponse(int status) {
        ...
        synchronized (WebRefinerTestServer.class) {
            if (sReasons == null) {
    ...
    private static class ServerThread extends Thread {
        private WebRefinerTestServer mServer;
        ...
        public ServerThread(WebRefinerTestServer server, int port, boolean ssl) throws Exception {
            super("ServerThread");
            mServer = server;
            mIsSsl = ssl;
            int retry = 3;
