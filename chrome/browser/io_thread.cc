...
IOThread::IOThread(
  ...
  registrar_->Add(prefs::kEnterpriseTrust,
                  base::Bind(&IOThread::AddTrust, base::Unretained(this)));
...
void IOThread::RegisterPrefs(PrefRegistrySimple* registry) {
  ...
  registry->RegisterStringPref(prefs::kEnterpriseTrust, std::string());
}
...
void IOThread::AddTrust() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);

  const PrefService* prefs = registrar_->prefs();

  if (prefs != NULL) {
    std::string trust = prefs->GetString(prefs::kEnterpriseTrust);

    BrowserThread::PostTask(
        BrowserThread::IO,
        FROM_HERE,
        base::Bind(&IOThread::AddTrustOnIOThread,
                   base::Unretained(this), trust));
  }
}

void IOThread::AddTrustOnIOThread(const std::string& b64trust) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);

#if defined(OS_ANDROID)
  std::string trust;
  if (base::Base64Decode(b64trust, &trust)) {
      net::android::AddEnterpriseRootCertificate((const uint8_t*)trust.data(),
                                                 trust.size());
  }
#endif
}
