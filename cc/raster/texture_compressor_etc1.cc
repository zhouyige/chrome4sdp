...
template<class ColorType> inline ColorType MakeColor(const ColorType& base, int16_t lum) {
...
template<class ColorType> inline uint32_t GetColorError(const ColorType& u, const ColorType& v) {
...
template<class ColorType> void GetAverageColor(const ColorType* src, float* avg_color) {
...
template<class ColorType> void ComputeLuminance(uint8_t* block,
                      const ColorType* src,
                      const ColorType& base,
                      int sub_block_id,
  ...
  for (unsigned int tbl_idx = 0; tbl_idx < 8; ++tbl_idx) {
    ...
    ColorType candidate_color[4];  // [modifier]
    ...
    for (unsigned int i = 0; i < 8; ++i) {
      ...
      for (unsigned int mod_idx = 0; mod_idx < 4; ++mod_idx) {
        const ColorType& color = candidate_color[mod_idx];
 ...
 */
template<class ColorType> bool TryCompressSolidBlock(uint8_t* dst, const ColorType* src) {
  ...
  ColorType base = MakeColor555Template<ColorType>(src_color_float);
  ...
  for (unsigned int tbl_idx = 0; tbl_idx < 8; ++tbl_idx) {
    ...
    for (unsigned int mod_idx = 0; mod_idx < 4; ++mod_idx) {
      int16_t lum = g_codeword_tables[tbl_idx][mod_idx];
      const ColorType& color = MakeColor(base, lum);
...
template<class ColorType> void CompressBlock(uint8_t* dst, const ColorType* ver_src, const ColorType* hor_src) {
  if (TryCompressSolidBlock(dst, ver_src))
    return;

  const ColorType* sub_block_src[4] = {ver_src, ver_src + 8, hor_src, hor_src + 8};

  ColorType sub_block_avg[4];
  ...
  for (unsigned int i = 0, j = 1; i < 4; i += 2, j += 2) {
    ...
    ColorType avg_color_555_0 = MakeColor555Template<ColorType>(avg_color_0);

    float avg_color_1[3];
    GetAverageColor(sub_block_src[j], avg_color_1);
    ColorType avg_color_555_1 = MakeColor555Template<ColorType>(avg_color_1);
    ...
    for (unsigned int light_idx = 0; light_idx < 3; ++light_idx) {
      ...
      if (component_diff < -4 || component_diff > 3) {
        ...
        sub_block_avg[i] = MakeColor444Template<ColorType>(avg_color_0);
        sub_block_avg[j] = MakeColor444Template<ColorType>(avg_color_1);
      } else {
...
template<class ColorType> void DoCompress(const uint8_t* src,
                                     uint8_t* dst,
                                     int width,
                                     int height,
  ...
  ColorType ver_blocks[16];
  ColorType hor_blocks[16];

  int block_width = width - (width % 4);
  int block_height = height - (height % 4);

  for (int y = 0; y < height; y += 4, src += width * 4 * 4) {
    for (int x = 0; x < width; x += 4, dst += 8) {
      const ColorType* row0 = reinterpret_cast<const ColorType*>(src + x * 4);
      const ColorType* row1 = row0 + width;
      const ColorType* row2 = row1 + width;
      const ColorType* row3 = row2 + width;

      if (y == block_height && block_height < height) {
        // Pad block with last row
        int remaining = height - block_height;
        row1 = remaining > 1 ? row1 : row0;
        row2 = remaining > 2 ? row2 : row1;
        row3 = row2;
      }

      if (x == block_width && block_width < width) {
        // Pad block with last column
        int remaining = width - block_width;

        // Copy available colors
        memcpy(hor_blocks, row0, 4 * remaining);
        memcpy(hor_blocks + 4, row1, 4 * remaining);
        memcpy(hor_blocks + 8, row2, 4 * remaining);
        memcpy(hor_blocks + 12, row3, 4 * remaining);

        // Repeat final color
        for (int i = remaining; i < 4; i++) {
          hor_blocks[i] = hor_blocks[i - 1];
          hor_blocks[i + 4] = hor_blocks[i + 3];
          hor_blocks[i + 8] = hor_blocks[i + 7];
          hor_blocks[i + 12] = hor_blocks[i + 11];
        }
      }
      else {
      ...
      // Copy from hor_blocks to ver_blocks
      memcpy(ver_blocks, hor_blocks, 8);
      memcpy(ver_blocks + 2, hor_blocks + 4, 8);
      memcpy(ver_blocks + 4, hor_blocks + 8, 8);
      memcpy(ver_blocks + 6, hor_blocks + 12, 8);
      memcpy(ver_blocks + 8, hor_blocks + 2, 8);
      memcpy(ver_blocks + 10, hor_blocks + 6, 8);
      memcpy(ver_blocks + 12, hor_blocks + 10, 8);
      memcpy(ver_blocks + 14, hor_blocks + 14, 8);
...
template<>
void TextureCompressorETC1<Color>::Compress(const uint8_t* src,
                                     uint8_t* dst,
                                     int width,
                                     int height,
                                     Quality quality) {
  DoCompress<Color>(src, dst, width, height, quality);
}

template<>
void TextureCompressorETC1<RgbaColor>::Compress(const uint8_t* src,
                                     uint8_t* dst,
                                     int width,
                                     int height,
                                     Quality quality) {
  DoCompress<RgbaColor>(src, dst, width, height, quality);
}
