...
TEST(SimpleMap, General) {
  ...
  simple_map<int, int>::iterator iter(m.begin());
  ...
  const simple_map<int, int>& ref = m;
  EXPECT_TRUE(ref.find(1234) != m.end());
  EXPECT_TRUE(ref.find(5678) == m.end());
}
TEST(SimpleMap, PostFixIteratorIncrement) {
  simple_map<int, int> m;
  m[0] = 5;
  m[2] = 3;
  ...
  {
    simple_map<int, int>::iterator iter(m.begin());
    simple_map<int, int>::iterator last(iter++);
    ++last;
    EXPECT_TRUE(last == iter);
  }

  {
    simple_map<int, int>::const_iterator iter(m.begin());
    simple_map<int, int>::const_iterator last(iter++);
    ++last;
    EXPECT_TRUE(last == iter);
  }
}
...
TEST(SimpleMap, CopyConstructor) {
  simple_map<int, int> src;

  {
    simple_map<int, int> m(src);
    EXPECT_TRUE(m.empty());
  }
...
static bool SimpleMapIsSubset(simple_map<K, V> const& a,
                             simple_map<K, V> const& b) {
  typename simple_map<K, V>::const_iterator it;
  for (it = a.begin(); it != a.end(); ++it) {
    typename simple_map<K, V>::const_iterator it_in_b = b.find(it->first);
...
template<class K, class V>
static bool SimpleMapEqual(simple_map<K, V> const& a,
                          simple_map<K, V> const& b) {
  return SimpleMapIsSubset(a, b) && SimpleMapIsSubset(b, a);
}

TEST(SimpleMap, AssignmentOperator) {
  simple_map<int, int> src_small;
  simple_map<int, int> src_large;
  ...
  simple_map<int, int> dest_small;
  dest_small = src_small;
  EXPECT_TRUE(SimpleMapEqual(dest_small, src_small));
  //EXPECT_EQ(dest_small.UsingFullMap(),
  //          src_small.UsingFullMap());

  simple_map<int, int> dest_large;
  dest_large = src_large;
  EXPECT_TRUE(SimpleMapEqual(dest_large, src_large));
  ...
  EXPECT_TRUE(SimpleMapEqual(dest_small, src_large));
  //EXPECT_EQ(dest_small.UsingFullMap(),
  //          src_large.UsingFullMap());

  dest_large = src_small;
  EXPECT_TRUE(SimpleMapEqual(dest_large, src_small));
  //EXPECT_EQ(dest_large.UsingFullMap(),
  //          src_small.UsingFullMap());

  // Double check that SimpleMapEqual works:
  dest_large[42] = 666;
  EXPECT_FALSE(SimpleMapEqual(dest_large, src_small));
}

TEST(SimpleMap, Insert) {
  simple_map<int, int> sm;

  // loop through the transition from small map to map.
  for (int i = 1; i <= 10; ++i) {
    VLOG(1) << "Iteration " << i;
    // insert an element
    std::pair<simple_map<int, int>::iterator,
        bool> ret;
    ...
    for (int j = 1; j <= i; ++j) {
      simple_map<int, int>::iterator it = sm.find(j);
...
TEST(SimpleMap, Erase) {
  simple_map<std::string, int> m;
  simple_map<std::string, int>::iterator iter;
...
#if 0 //simple_map is an ordered map
TEST(SimpleMap, NonHashMap) {
  simple_map<int, int> m;
  EXPECT_TRUE(m.empty());
...
TEST(SimpleMap, SubclassInitializationWithFunctionPointer) {
  simple_map_add_item, 4, std::equal_to<int>,
...
TEST(SimpleMap, SubclassInitializationWithFunctionObject) {
  simple_map_add_item, 4, std::equal_to<int>,
...
TEST(SimpleMap, MoveOnlyValueType) {
  simple_map<int, MoveOnlyType> m;

  m[0] = MoveOnlyType(1);
  m[1] = MoveOnlyType(2);
  m.erase(m.begin());

  // SimpleMap will move m[1] to an earlier index in the internal array.
  EXPECT_EQ(m.size(), 1u);
  EXPECT_EQ(m[1].value(), 2);

  m[0] = MoveOnlyType(1);
  // SimpleMap must move the values from the array into the internal std::map.
  m[2] = MoveOnlyType(3);
  ...
  // SimpleMap should also let internal std::map erase with a move-only type.
  EXPECT_EQ(m.size(), 2u);
  EXPECT_EQ(m[1].value(), 2);
  EXPECT_EQ(m[2].value(), 3);
}
