...
bool ResourcePrefetchPredictor::URLRequestSummary::SummarizeResponse(
  ...
  summary->navigation_id = NavigationID(render_process_id, info->GetRouteID(), render_frame_id, info->GetParentRenderFrameID(),
...
void ResourcePrefetchPredictor::StartPrefetching(const NavigationID& navigation_id) {
  ...
  if (!GetPrefetchData(navigation_id.main_frame_url, &urls)) {
...
void ResourcePrefetchPredictor::OnMainFrameRequest(
  ...
  inflight_navigations_.insert(
      std::make_pair(request.navigation_id,
                     base::MakeUnique<PageRequestSummary>(
                       request.navigation_id.main_frame_url)));
}
...
bool ResourcePrefetchPredictor::PopulatePrefetcherRequest(
  ...
  for (const ResourceData& resource : it->second.resources()) {
    ...
    urls->push_back(ResourcePrefetcher::URLInfo(
